...
class MatMulOp(Op):
    ...
    def gradient(self, node, output_grad):
        ...
        if (node.matmul_attr_trans_A is False and node.matmul_attr_trans_B is False):
            ...
            dA = matmul_op(node.inputs[1], output_grad, trans_A=False, trans_B=True)
            dB = matmul_op(node.inputs[0], output_grad, trans_A=True, trans_B=False)
        elif (node.matmul_attr_trans_A is False and node.matmul_attr_trans_B is True):
            dA = matmul_op(output_grad, node.inputs[1], trans_A=False, trans_B=True)
            dB = matmul_op(output_grad, node.inputs[0], trans_A=True, trans_B=False)
        elif (node.matmul_attr_trans_A is True and node.matmul_attr_trans_B is True):
            dA = matmul_op(node.inputs[1], output_grad, trans_A=False, trans_B=True)
            dB = matmul_op(output_grad, node.inputs[0], trans_A=True, trans_B=False)
        return [dA, dB]
...
class Executor:
    ...
    def run(self, feed_dict):
        ...
        for node in topo_order:
            # check if the node is placeholder op, if so, skip
...
def gradients(output_node, node_list):
    ...
    for node in reverse_topo_order:
        # sum partial adjoints from the output edges
        output_grad = sum_node_list(node_to_output_grads_list[node])
        #print("node", node)
        node_to_output_grad[node] = output_grad
        # get the gradient of the node derivate to node.inputs
        input_grads_list = node.op.gradient(node, output_grad)
        #print(input_grads_list)
        #print(node_to_output_grads_list)
        #print(node_to_output_grad)
        # add input grads to node_to_grad, when first include, create empty value list for the key
        # to avoid key error.
        for i in range(len(node.inputs)):
            #print(i, node.inputs[i])
